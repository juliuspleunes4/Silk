...
"#;
    let result = analyze(source);
    assert!(result.is_ok(), "Default can use variable from outer scope");
}
...
"#;
    let result = analyze(source);
    assert!(result.is_ok(), "Multiple defaults can use outer variables and expressions");
}
...
def foo(x=get_default()):
    pass
"#;
    let result = analyze(source);
    assert!(result.is_ok(), "Default can call function from outer scope");
}
