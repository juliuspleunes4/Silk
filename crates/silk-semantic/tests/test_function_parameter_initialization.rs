...
/// parameter initialization, not whether test functions are called.
fn analyze(source: &str) -> Result<(), Vec<SemanticError>> {
    ...
    // Filter out UnusedFunction and UnusedVariable errors - we're testing parameter initialization
    match result {
        Ok(()) => Ok(()),
        Err(errors) => {
