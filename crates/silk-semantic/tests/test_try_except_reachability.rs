...
/// try/except reachability analysis, not whether test functions are called.
fn analyze_control_flow(source: &str) -> Result<(), Vec<SemanticError>> {
    ...
    // Filter out UnusedFunction and UnusedVariable errors - we're testing try/except reachability
    match result {
        Ok(()) => Ok(()),
        Err(errors) => {
